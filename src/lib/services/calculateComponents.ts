...
function calculateRawAndExcessComponents(
  recipeTree: RecipeTree,
  timesToProduce: number
): { raw: Map<string, number>; excess: Map<string, number> } {
  ...
  for (const requiredRecipeEntity of recipe.required) {
    for (const requiredTree of recipeTree.required) {
      if (requiredTree.stuff === requiredRecipeEntity.stuff) {
        const requiredTimesToProduce = getTimesToProduce(
          requiredTree,
          timesToProduce * requiredRecipeEntity.count
        );
        const calculatedRequired = calculateRawAndExcessComponents(
          requiredTree,
          requiredTimesToProduce
        );
  ...
  for (const producedEntity of recipe.produced) {
    if (producedEntity.stuff !== recipeTree.stuff) {
      addToMap(
        excess,
        producedEntity.stuff,
        producedEntity.count * timesToProduce
      );
    }
  }
...
function calculateInitialComponents(
  recipeTree: RecipeTree,
  timesToProduce: number
): RecipeEntity[] {
  const recipe = recipeTree.selectedRecipe;
  return recipe.required.map((e) => ({
    stuff: e.stuff,
    count: e.count * timesToProduce,
  }));
}
...
export function calculateComponents(
  ...
  const components = calculateRawAndExcessComponents(
    recipeTree,
    timesToProduce
  );
  ...
  const excessResult =
    excessResultCount > 0
      ? [{ stuff: recipeTree.stuff, count: excessResultCount }]
      : [];
