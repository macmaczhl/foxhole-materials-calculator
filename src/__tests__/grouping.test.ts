import { getItemGroup, createGroupedStuffList, ItemGroup, Vehicles, RawResources, Materials, Liquids } from '@/lib/models';
...
describe('Item Grouping', () => {
  describe('getItemGroup', () => {
    ...
    test('categorizes raw resources correctly', () => {
      expect(getItemGroup(RawResources.Components)).toBe(ItemGroup.RawResources);
      expect(getItemGroup(RawResources.Coke)).toBe(ItemGroup.RawResources);
    });

    test('categorizes processed materials as components', () => {
      expect(getItemGroup(Materials.ConstructionMaterials)).toBe(ItemGroup.Components);
      expect(getItemGroup(Materials.RefinedMaterials)).toBe(ItemGroup.Components);
      expect(getItemGroup(Materials.AssemblyMaterialsI)).toBe(ItemGroup.Components);
  ...
  describe('createGroupedStuffList', () => {
    ...
    test('components group contains processed materials', () => {
      const grouped = createGroupedStuffList();
      const componentsGroup = grouped.find(g => g.group === ItemGroup.Components);
